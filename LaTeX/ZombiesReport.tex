...
\[= \frac{(- \gamma) \pm \sqrt{\gamma ^2}}{2} = \frac{(- \gamma) \pm \sqrt{\gamma ^2}}{2}\]
...
\item[$R_0 = 1$] Each cooked student would influence one healthy student on average, leading to a final state where removed and healthy populations are equal in size.
\item[$R_0 > 1$] Each cooked student is able to influence more than one healthy student, leading the final healthy student population less than than the removed population.
\end{description}
...
Additionally, in the phase space diagrams, the nullcline shifts to the right by $\alpha$, which can be seen  by the sudden change in direction of the arrows either side of the nullcline.
...
\begin{figure}[h]
\centering
  \includegraphics[width=0.7\linewidth]{Figures/Alt1/bif.png}
  \caption{Effect of varying reproduction rate ($R_0$) on final populations.}
\end{figure}


This shows that if the transmission rate $R_0$ is less than 1, then waiting for all of the unhealthy students' habits to just die out is a viable strategy without losing any students. If poor academic habits spread faster than unhealthy students drop out, then the total student body will end up dropping out unless further interventions are applied.
...
\centering
\begin{subfigure}{0.45\textwidth}
\includegraphics[width=0.9\linewidth, height=6cm]{Figures/Alt2/timepop,al,0.5} 
\caption{$\alpha = 0.5$}
\end{subfigure}
\begin{subfigure}{0.45\textwidth}
\includegraphics[width=0.9\linewidth, height=6cm]{Figures/Alt2/timepop,al,0.7} 
\caption{$\alpha = 0.7$}
\end{subfigure}


\caption{Impact of increasing $\alpha$ from 0.5 to 0.7.}
\end{figure}


(not sure how to word the robustness testing, but I did the monte carlo ensemble ?)
...
\caption{Phase space plots showing sinkhole (H vs C) and equilibrium line (C v. R)}
\end{figure}

\textcolor{red}{I need some good explanation of these graph shapes here, I don't know what the shapes are called or how to explain them}

\subsection{Control Parameter / Threshold}

Another graph can be created to visualise the final populations when varying reproduction rate, however since $\gamma$ no longer permanently removes zombies, the our basic reproduction number now becomes;

\[R_0 = \frac{\beta_c}{\alpha}\]
...
Now, $\delta$ effects the final equilibrium values of the populations as well as the convergence speed, but once again does not shift the bifurcation point.


The system has a single stable unhealthy habit free fixed point (C = 0), however as $R_0$ increases past the critical threshold ($R_0 = 1$), a transcritical bifurcation occurs, with a new stable state emerging. The Cooked population is now able to exist indefinitely (endemic equilibrium).
...
\centering
  ...
  \caption{Sensitivity analysis of 'Mitigating Circumstances'}
\end{figure}

As the return rate $\delta$ increases, the stable state population of unhealthy student population also increases. This correlates well to real-world application, as the more Imperial is able to encourage struggling students to keep going without actually improving their long-term situations, the more unhealthy students will be stuck in a state of needing repeated support interventions.
...
\begin{description}
\item[Maximise well-being resources] The model reveals that maximising $\alpha$ is the critical metric, determining the value at which the healthy student population tends to. Therefore, maximising $\alpha$ is far more valuable than maxmimising $\delta$ if total eradication of unhealthy habits is desired. In section 4.3, we discovered that the population is strictly limited by $\alpha$. Therefore, to ensure that at least 80\% of the student body maintains healthy working habits, the university must sufficiently fund long-term interventions for students. This could include well-being meetings, mental health support, mentoring, study skills building workshops, etc. Merely mitigating drop-out by increasing $\delta$ without increasing $\alpha$ will result in an academic environment rife with poor study habits being—high student retention, very low student well-being \& academic performance. 
\end{description}

For future improvements to the model;
\begin{description}
\item[Improve Robustness] Although Monte Carlo ensemble simulations were run, improvements can be made to the stability of the model. The model is robust to small fluctuations in initial student numbers, however the model breaks down when noise is > 0.03, which is not robust enough to adequately rely on in real-world scenarios. Although the percentage change of students over one year is only a 3\% increase, the increase over the last five years is over 14\%.
